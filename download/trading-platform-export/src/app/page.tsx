...
import { StrategyView } from '@/components/trading/strategy-view'
import { TradingHubView } from '@/components/trading/trading-hub-view'
import { LogTradeView } from '@/components/trading/log-trade-view'
...
export default function Home() {
  ...
  const handleSignup = async (data: { email: string; password: string; name: string }) => {
    ...
    setUser(newUser)
    localStorage.setItem('trademaster_user', JSON.stringify(newUser))
    setAppView('dashboard')
    setIsLoading(false)
    ...
    toast({
      ...
      description: language === 'ar' 
        ? `مرحباً ${data.name}، تم إنشاء حسابك بنجاح` 
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'zen-mode':
        return <ZenModeView />
      case 'pricing':
        return <PricingView onSelectPlan={handleSelectPlan} currentPlan={user?.plan} />
      default:
        return <DashboardView />
    }
  }
